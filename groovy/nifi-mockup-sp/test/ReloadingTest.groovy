...
class Greeter {
    String sayHello() {
		...
		def greet = Dependency.sayHello()
        greet
    }
	
    String sayDB(Sql mySql) {
		def sql = mySql
		...
		//return data
		'<br> Test from groovy file!! <br>' + ' temp table person odometer: '+ tPerson + ('<br> firstRow ->' + fr1.name + ' ha ha ' + fr1.city + '' ) + er2 + '<br>'
		// + cron + '<br>'
    }
}
